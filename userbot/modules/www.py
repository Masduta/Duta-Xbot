...
async def redis(pong):
    ...
    await pong.edit("**✲**")
    await pong.edit("**✲✲**")
    await pong.edit("__DUAR KEMEM__")
...
async def redis(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("__xping🔥.__")
    await pong.edit("__mengecek..__")
    await pong.edit("__Sabar Ngentod.__")
    await pong.edit("__😡...__")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**👻 Duta-Xbot 👻**\n"
                    f"➾ __Signal__    __:__ "
                    f"`%sms` \n"
                    f"➾ __Uptime__ __:__ "
                    f"`{uptime}` \n" % (duration))
...
async def redis(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("**Duta-Xbot**")
    await pong.edit("**✿**")
    await pong.edit("**✿✿**")
    await pong.edit("**Ping!!Pong!!**")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**👻Duta-Xping👻**\n"
                    f"❊ **Ping:** "
                    f"`%sms` \n"
                    f"❊ **Uptime:** "
                    f"`{uptime}` \n"
...
async def pingme(pong):
    ...
    await pong.edit(f"**👻OwnerName👻 : {ALIVE_NAME}**\n📗 `%sms`" % (duration))
